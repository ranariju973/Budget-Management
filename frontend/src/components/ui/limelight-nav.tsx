    ...
    >
      {items.map(({ id, icon, label, onClick }, index) => (
          <a
            ...
            className={`relative z-20 flex flex-1 min-w-0 h-full cursor-pointer items-center justify-center ${iconContainerClassName || ''}`}
      ...
      <div 
        ...
        style={{
          ...
          boxShadow: '0 50px 15px var(--color-accent)',
        }}
      >
        <div
          className="absolute left-[-30%] top-[5px] w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] pointer-events-none"
          style={{
            background: 'linear-gradient(to bottom, color-mix(in srgb, var(--color-accent) 30%, transparent), transparent)',
          }}
        />
      </div>
    </nav>
